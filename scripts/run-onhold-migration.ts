/**
 * Run On Hold Status Migration
 * This script executes the SQL migration to add ON_HOLD status and onHold fields
 */
...
async function runMigration() {
  try {
    console.log('📊 Starting ON_HOLD status migration...\n');

    // Read the SQL file
    const sqlFilePath = path.join(__dirname, '../prisma/migrations/add_onhold_status_and_update_fields/migration.sql');
    ...
    console.log('✅ ON_HOLD status migration completed successfully!');
    console.log('\n📈 Changes applied:');
    console.log('  - Added ON_HOLD to PayrollStatus enum');
    console.log('  - Added onHold tracking fields to Payroll table');
    console.log('  - Migrated existing REJECTED records to PENDING for Finance to fix');
  } catch (error) {
